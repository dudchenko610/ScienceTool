...
core::Example_2_3_Window::Example_2_3_Window(const int w, const int h, const char* l) : Window(w, h, l),
...
core::Example_2_3_Window::~Example_2_3_Window()
{
	delete m_Program;
	delete m_VBA;
	delete m_VBO;
...
void core::Example_2_3_Window::Init()
{
	...
	m_Program = new Program("res/shaders/opengl-cookbook/chapter2/ex_3/Subroutine.shader");
	...
	lightInfo.Ls = glm::vec3(0.1f, 0.2f, 0.9f);
	...
	this->adsIndex = glGetSubroutineIndex( 
		((opengl::Bindable*) m_Program)->GetId(), 
		GL_VERTEX_SHADER, 
		"phongModel" 
	);

	this->diffuseIndex = glGetSubroutineIndex(
		((opengl::Bindable*) m_Program)->GetId(),
		GL_VERTEX_SHADER,
		"diffuseOnly"
	);
...
void core::Example_2_3_Window::Loop(float dt)
{
	time += dt;
	...
	// render ads shading model
	m_Program->SetUniformMat4f("u_Model", glm::mat4(glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.0f, -2.0))));
	glUniformSubroutinesuiv(GL_VERTEX_SHADER, 1, &this->adsIndex);
	GLCall(glDrawArrays(GL_TRIANGLES, 0, this->trianglesCount * 3));

	// render diffuse only shading model
	m_Program->SetUniformMat4f("u_Model", glm::mat4(glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.0f, 2.0))));
	glUniformSubroutinesuiv(GL_VERTEX_SHADER, 1, &this->diffuseIndex);
	GLCall(glDrawArrays(GL_TRIANGLES, 0, this->trianglesCount * 3));
...
void core::Example_2_3_Window::OnKeyAction(int key, int scancode, int action, int mods)
{
...
void core::Example_2_3_Window::OnMouseMoveAction(double xpos, double ypos)
{
}

void core::Example_2_3_Window::OnMouseButtonAction(int button, int action, int mods)
{
}

void core::Example_2_3_Window::OnScrollAction(double xoffset, double yoffset)
{
}
